...
export function ContributionMemoCard({
  ...
  return (
    ...
    <div className={cn('flex w-full flex-col', className)}>
      <button
        type="button"
        className="group/memo relative w-full rounded-lg overflow-hidden border border-border bg-card min-h-[180px] cursor-pointer hover:ring-2 hover:ring-primary/50 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring text-left"
      ...
      {reactionsEnabled && (
        <ReactionBar id={reactionId ?? `memo:${title}`} className="px-0.5 pt-2" />
      )}
    </div>
  );
}
