...
export function ContributionWhiteboardCard({
  ...
  return (
    ...
    // a <button>, and interactive controls can't nest inside one.
    <div className={cn('flex w-full flex-col', className)}>
      <button
        type="button"
        className="group/wb relative w-full rounded-lg overflow-hidden border border-border bg-muted/30 h-[200px] cursor-pointer hover:ring-2 hover:ring-primary/50 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring text-left"
      ...
      {reactionsEnabled && (
        <ReactionBar id={reactionId ?? `whiteboard:${title}`} className="px-0.5 pt-2" />
      )}
    </div>
  );
}
