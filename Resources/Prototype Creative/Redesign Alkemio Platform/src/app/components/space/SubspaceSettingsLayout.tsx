...
const PhaseColumn = ({
  ...
  return (
    <div className="flex items-start">
      <Collapsible open={isOpen} onOpenChange={onToggle} className="flex-1 min-w-0">
        <div
          className={cn(
          ...
          <div className="px-3 py-3 bg-muted/30 space-y-2">
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  {!isLast && (
                    <DropdownMenuItem onClick={() => onToggleLink(phase.id)}>
                      {phase.linkedToNext ? (
                        <>
                          <Unlink className="w-4 h-4 mr-2" />
                          Unlink from next phase
                        </>
                      ) : (
                        <>
      ...
      {/* Link toggle between phases — clickable */}
      {!isLast && (
        <div className="flex flex-col items-center justify-start w-10 shrink-0 pt-4 gap-1">
          <button
            onClick={() => onToggleLink(phase.id)}
            className={cn(
              "w-7 h-7 rounded-full flex items-center justify-center transition-all border",
              phase.linkedToNext
                ? "bg-primary/10 border-primary/30 text-primary hover:bg-primary/20"
                : "bg-muted/50 border-border text-muted-foreground/40 hover:border-muted-foreground/40 hover:text-muted-foreground"
            )}
            title={phase.linkedToNext ? "Click to unlink phases" : "Click to link phases"}
          >
            {phase.linkedToNext ? (
          ...
          <span className="text-[9px] text-muted-foreground/50 select-none">
            {phase.linkedToNext ? "linked" : "link"}
          </span>
        </div>
      )}
    </div>
...
export function SubspaceSettingsLayout() {
  ...
  const handleToggleLink = (id: string) => {
    setPhases((prev) =>
      prev.map((p) => (p.id === id ? { ...p, linkedToNext: !p.linkedToNext } : p))
    );
  };
    ...
    <DndProvider backend={HTML5Backend}>
      <div className="w-full h-full">
        <div className="flex flex-col h-full">
          {/* Header */}
          <div className="mb-6">
            <h2 className="text-xl font-semibold text-foreground">Innovation Flow</h2>
            <p className="text-sm text-muted-foreground mt-1">
              Design your subspace's innovation flow. Add, remove, reorder, and link phases. Drag posts between phases.
            </p>
          </div>
                ...
                {i < phases.length - 1 && (
                  <span className="text-muted-foreground">
                    {phase.linkedToNext ? (
                      ...
                      <span className="inline-block w-3 h-[1px] bg-border" />
                    )}
                  </span>
                )}
              </div>
