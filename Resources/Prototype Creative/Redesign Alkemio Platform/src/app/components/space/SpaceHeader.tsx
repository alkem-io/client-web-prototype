...
import { Settings, Share2, Video, FileText, MessageCircle } from "lucide-react";
...
export function SpaceHeader({ spaceSlug, onOpenChat }: SpaceHeaderProps) {
        ...
        <div className="relative h-full container mx-auto px-6 py-8 flex flex-col justify-between pb-12">
          ...
          <div className="flex justify-end">
            <div className="flex items-center gap-2">
              {/* Space Chat trigger — Variant A & B */}
              {(variant === "A" || variant === "B") && onOpenChat && (
                <Button
                  variant="ghost"
                  size="icon"
                  className="relative text-white/80 hover:text-white hover:bg-white/20"
                  onClick={onOpenChat}
                  aria-label={`Open Space chat${chatUnread > 0 ? ` (${chatUnread} unread)` : ""}`}
                >
