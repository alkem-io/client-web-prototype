          ...
          <div className="flex flex-wrap items-center gap-2">
            <Badge 
              ...
              className={cn(
                ...
                template.type === "Space" ? "bg-purple-50 text-purple-700 border-purple-200" : 
                template.type === "Subspace" ? "bg-indigo-50 text-indigo-700 border-indigo-200" :
  ...
  return (
    <div className="bg-white rounded-xl border border-border overflow-hidden flex flex-col h-full shadow-sm min-h-[500px]">
      ...
      <div className="bg-white border-b border-border">
          <div className="h-32 bg-gradient-to-r from-purple-50 to-indigo-50 w-full relative">
             ...
             {activeTab === "About" && (
                 <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                     <div className="bg-white p-6 rounded-lg border border-border shadow-sm">
                         ...
                         <p className="text-slate-600 leading-relaxed">{structure.about || "This space serves as a central hub for collaboration."}</p>
                     </div>
             ...
             {activeTab === "Resources" && (
                 <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                     <div className="bg-white rounded-lg border border-border shadow-sm overflow-hidden">
                         <div className="p-4 border-b border-border bg-slate-50 font-medium text-sm text-slate-700">Files & Links</div>
                         <div className="divide-y divide-border">
                             {structure.samplePosts?.["Resources"]?.map((res: any, i: number) => (
                                 <div key={i} className="p-4 flex items-center gap-3 hover:bg-slate-50 transition-colors">
                                     {res.type === "File" ? <FileText className="w-5 h-5 text-blue-500" /> : <ExternalLink className="w-5 h-5 text-green-500" />}
                                     <div className="flex-1">
                                         <div className="text-sm font-medium text-slate-900">{res.title}</div>
                                         <div className="text-xs text-slate-500">{res.type} • Added recently</div>
                                     </div>
                                 </div>
                             ))}
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                         {structure.templates?.map((temp: any, i: number) => (
                             <div key={i} className="bg-white p-4 rounded-lg border border-border shadow-sm flex flex-col gap-2">
                                 ...
                                 <div className="font-medium text-slate-900">{temp.name}</div>
                                 <div className="text-sm text-slate-500">Ready to use in this space.</div>
    ...
    <div className="bg-white rounded-xl border border-border overflow-hidden shadow-sm">
       <div className="p-6 border-b border-border bg-white">
          <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                  <h3 className="text-lg font-semibold text-slate-900">Innovation Flow</h3>
                  <p className="text-slate-500 text-xs">Subspace Structure</p>
              </div>
          </div>
          <p className="text-slate-600 text-sm mt-2 max-w-3xl">{structure.description}</p>
       </div>
             ...
             {structure.stages?.map((stage: any, i: number) => (
                 <div key={i} className="relative pl-8 md:pl-0">
                     ...
                     {i < structure.stages.length - 1 && (
                        <div className="absolute left-[15px] top-10 bottom-[-20px] w-0.5 bg-slate-200 md:hidden" />
                     )}

                     <div className="bg-white border border-border rounded-lg shadow-sm overflow-hidden">
                         <div className="p-4 flex items-start gap-4 border-b border-slate-50">
                             <div className="w-8 h-8 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-sm font-bold text-slate-600 shrink-0">
                                 {i + 1}
                             </div>
                             <div className="flex-1">
                                 <h4 className="font-semibold text-slate-900">{stage.name}</h4>
                                 <p className="text-xs text-slate-500 mb-3">{stage.posts?.length || 0} items included</p>
                                 
                                 <div className="space-y-2">
                                     {stage.posts?.map((post: string, k: number) => (
                                         <div key={k} className="flex items-center gap-2 text-sm text-slate-600 bg-slate-50 p-2 rounded border border-slate-100">
...
function RenderPostContent({ structure }: { structure: any }) {
  ...
  // Use title or titleTemplate as title
       ...
       <div className="bg-white rounded-xl border border-border shadow-sm overflow-hidden">
           ...
           <div className="p-6">
               <div className="flex items-start gap-3 mb-4">
                   ...
                   <div className="flex-1">
                       <div className="flex items-center justify-between">
                           <div className="font-semibold text-sm text-slate-900">Author Name</div>
                           <MoreHorizontal className="w-4 h-4 text-slate-400" />
                       </div>
                       <div className="text-xs text-slate-500">Just now • in General</div>
                   </div>
               </div>
       ...
       <div className="bg-white rounded-xl border border-border shadow-sm overflow-hidden">
           ...
           <div className="p-6">
               <div className="flex items-start gap-3 mb-4">
                   ...
                   <div className="flex-1">
                       <div className="flex items-center justify-between">
                           <div className="font-semibold text-sm text-slate-900">Alkemio User</div>
                           <MoreHorizontal className="w-4 h-4 text-slate-400" />
                       </div>
               ...
               <div className="space-y-4 mb-6">
                   <h2 className="text-lg font-bold text-slate-900">{structure.post?.title || "Post Title"}</h2>
                   <div className="prose prose-sm prose-slate max-w-none text-slate-600">
               ...
               <div className="mt-4">
                   <div className="bg-white rounded-lg border border-slate-200 overflow-hidden hover:border-primary/50 transition-all cursor-pointer group shadow-sm">
                       {/* Preview Area for Tool */}
                       <div className="h-40 bg-slate-100 relative flex items-center justify-center overflow-hidden">
                           <Grid className="absolute inset-0 text-slate-200 w-full h-full opacity-50" />
                           
                           {/* Mini visual representation based on type */}
                           {structure.component?.type === "Whiteboard" ? (
                               <div className="bg-white p-2 rounded shadow-sm border border-slate-200 absolute inset-4 flex flex-col gap-2">
                                   ...
                                   <div className="flex-1 relative">
                                       <div className="absolute top-4 left-4 w-12 h-12 bg-yellow-100 border border-yellow-200 rounded-sm shadow-sm rotate-1" />
                                       <div className="absolute top-8 right-8 w-16 h-10 bg-blue-100 border border-blue-200 rounded-sm shadow-sm -rotate-2" />
                                   </div>
                               </div>
                           ) : (
                               <div className="bg-white p-4 rounded shadow-sm border border-slate-200 absolute inset-8 flex flex-col">
                           ...
                           <div className="absolute bottom-2 right-2 bg-white/90 backdrop-blur px-2 py-1 rounded text-[10px] font-medium border border-slate-200 text-slate-600 flex items-center gap-1">
                       ...
                       <div className="p-3 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                           <div>
                               <div className="flex items-center gap-2 mb-0.5">
                                   <h4 className="font-medium text-slate-900 text-sm">{structure.component?.name}</h4>
                                   <Badge variant="outline" className="text-[10px] h-4 px-1 bg-white border-slate-200 text-slate-500">{structure.component?.type}</Badge>
                               </div>
                               <p className="text-xs text-slate-500 truncate max-w-[200px]">{structure.component?.preview}</p>
                           </div>
...
function RenderWhiteboardContent({ template }: { template: any }) {
  ...
  return (
    <div className="w-full h-[600px] bg-white rounded-xl overflow-hidden border border-border relative flex flex-col shadow-sm">
        {/* Mock Whiteboard Toolbar */}
        <div className="h-12 border-b border-border bg-slate-50 flex items-center justify-between px-4 shrink-0">
            <div className="flex items-center gap-2">
                <div className="p-1.5 rounded hover:bg-slate-200 cursor-pointer text-slate-600"><Monitor className="w-4 h-4" /></div>
                <div className="h-4 w-[1px] bg-slate-300 mx-1" />
                <div className="p-1.5 rounded hover:bg-slate-200 cursor-pointer text-slate-600"><PenTool className="w-4 h-4" /></div>
                <div className="p-1.5 rounded hover:bg-slate-200 cursor-pointer text-slate-600"><StickyNote className="w-4 h-4" /></div>
                <div className="p-1.5 rounded hover:bg-slate-200 cursor-pointer text-slate-600"><ImageIcon className="w-4 h-4" /></div>
            </div>
            <div className="text-xs font-medium text-slate-500">{template.name}</div>
            <div className="flex items-center gap-2">
        ...
        {/* Canvas Area */}
        <div className="flex-1 relative bg-slate-50 overflow-hidden cursor-move group">
              <div className="absolute inset-0 opacity-10 pointer-events-none">
                 <div className="w-full h-full" style={{ backgroundImage: 'radial-gradient(#94a3b8 1px, transparent 1px)', backgroundSize: '24px 24px' }}></div>
              </div>
              ...
              <div className="absolute inset-0 flex items-center justify-center transform group-hover:scale-105 transition-transform duration-700 ease-out">
                 ...
                 <div className="absolute top-1/4 left-1/4 transform -translate-x-1/2 -translate-y-1/2">
                     <div className="w-48 h-48 border-2 border-dashed border-slate-300 rounded-xl flex items-center justify-center bg-white/50">
                         <span className="text-slate-400 text-xs font-medium uppercase tracking-widest">Brainstorming Zone</span>
                     </div>
                     <div className="absolute -top-4 -right-4 w-32 h-32 bg-yellow-100 shadow-md rounded-sm transform rotate-6 border border-yellow-200 p-4 flex items-center justify-center text-center">
                         <span className="text-sm font-handwriting text-slate-700">How might we...?</span>
                     </div>
                 </div>

                 {/* Zone 2 */}
                 <div className="absolute bottom-1/3 right-1/4 transform translate-x-1/2 translate-y-1/2">
                     <div className="w-64 h-40 bg-white border border-slate-200 rounded shadow-lg p-4 flex flex-col gap-2">
                         ...
                         <div className="mt-auto flex gap-2">
                            ...
                            <div className="w-6 h-6 rounded-full bg-green-100 border border-green-200" />
                         </div>
                     </div>
                 </div>
              </div>
...
function RenderBriefContent({ structure }: { structure: any }) {
    ...
    return (
        <div className="bg-white rounded-xl border border-border overflow-hidden shadow-sm p-8">
            <div className="max-w-3xl mx-auto border border-slate-200 shadow-sm min-h-[600px] bg-white p-10 relative">
                {/* Document Header */}
                ...
                <div className="space-y-8">
                    {structure.sections?.map((section: any, i: number) => (
                        <div key={i} className="space-y-3">
                            <h4 className="text-sm font-bold text-slate-800 uppercase tracking-wide">{section.title}</h4>
                            <p className="text-sm text-slate-500 italic">{section.description}</p>
                            <div className="p-4 bg-slate-50 border border-slate-100 rounded border-dashed min-h-[60px] flex items-center justify-center text-xs text-slate-400">
                                {section.type} Input Area
                            </div>
                        </div>
                    ))}
                    {!structure.sections && (
                        <div className="text-center text-slate-400 py-12">No section data available</div>
                    )}
                </div>
            </div>
            <div className="flex justify-center mt-6">
...
function RenderCommunityGuidelines({ structure }: { structure: any }) {
    if (!structure) return null;
    return (
        <div className="bg-white rounded-xl border border-border overflow-hidden shadow-sm p-8 bg-slate-50/50">
    ...
    <div className="bg-white rounded-xl border border-border p-8 shadow-sm">
       <div className="max-w-2xl mx-auto">
           <div className="mb-6 pb-6 border-b border-border">
               <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Title Format</div>
               <div className="text-lg font-medium text-slate-900 font-mono bg-slate-50 p-3 rounded border border-slate-200 inline-block">
                   {structure?.titleTemplate || "[Title Here]"}
               </div>
           </div>
           
           <div className="space-y-6">
               <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Template Fields</div>
               {structure?.fields?.map((field: any, i: number) => (
                   <div key={i} className="space-y-2">
                       <label className="text-sm font-medium text-slate-700">{typeof field === 'string' ? field : field.label}</label>
                       <div className="h-10 bg-slate-50 border border-slate-200 rounded w-full" />
                   </div>
               ))}
               {!structure?.fields && <div className="text-slate-400 italic">No specific fields defined.</div>}
               
               {structure?.richText && (
                   <div className="flex items-center gap-2 text-xs text-slate-500 bg-blue-50 p-2 rounded text-blue-700 w-fit">
...
function MetadataPanel({ template }: { template: any }) {
  // Conditional Instructions: ONLY show if instructions exist
      ...
      {showInstructions && (
        <div className="bg-blue-50/50 rounded-xl border border-blue-100 overflow-hidden shadow-sm">
          <button 
            ...
            className="w-full flex items-center justify-between p-4 bg-blue-100/30 hover:bg-blue-100/50 transition-colors text-left border-b border-blue-100"
          >
             <h3 className="font-semibold text-blue-900 flex items-center gap-2 text-sm">
               <Info className="w-4 h-4 text-blue-600" />
               How to Use This Template
             </h3>
             {isInstructionsOpen ? <ChevronDown className="w-4 h-4 text-blue-500" /> : <ChevronRight className="w-4 h-4 text-blue-500" />}
          </button>
          
          {isInstructionsOpen && (
            <div className="p-5 animate-in slide-in-from-top-2 duration-200 bg-white/50">
               <div 
                 className="prose prose-sm prose-blue max-w-none text-slate-600 leading-relaxed"
                 dangerouslySetInnerHTML={{ __html: template.instructions }}
               />
               <p className="text-xs text-slate-400 mt-4 pt-4 border-t border-blue-100 italic">
      ...
      <div className="bg-card rounded-xl border border-border p-5 space-y-4 shadow-sm">
        ...
        <div>
           ...
           <div className="space-y-2">
              {template.type === "Collaboration Tool" && template.structure?.component ? (
                <>
                  <div className="text-sm text-slate-600 flex items-start gap-2 bg-slate-50 p-2 rounded">
